...
class LinePlotOneState {

    constructor(parentElement, data, state, metric) {
    ...
    initVis() {
        ...
        // Lockdown markers
        vis.lockdown = vis.svg.append('g')
            .attr('class', 'lockdown')

        vis.lockdown.append('line')
            .attr('x1', 290)
            .attr('y1', 0)
            .attr('x2', 290)
            .attr('y2', vis.height)
        ...
        vis.lockdown.append('line')
            ...
            .attr('stroke', 'black')
            .attr('stroke-width', 3)
            .attr('class', 'lockdown-end')

        vis.lockdown.append('text')
            .attr('x', 155)
            .attr('y', 20)
            .attr('class', 'lockdown-start-text')
    ...
    updateVis() {
        ...
        vis.field
            ...
            //     }
            // })
            .on('mouseover', function () {d3.select('.tool_tip').attr('display', null)})
            ...
            .on('mousemove', function(event, d) {
                ...
                vis.data_index = vis.bisectDate(vis.displayData.get(vis.state), vis.tooltip_date)
                console.log(vis.displayData.get(vis.state))
                d3.select('.tool_tip_state').text(vis.state)
                d3.select('.tool_tip_date').text(d3.timeFormat('%e %B %Y')(vis.tooltip_date))
                d3.select('.tool_tip_metric').text(d3.format(',')(vis.displayData.get(vis.state)[vis.data_index][vis.metric]))
            })
