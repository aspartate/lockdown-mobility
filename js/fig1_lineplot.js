...
class LinePlotAll {
    ...
    initVis() {
        ...
        vis.tool_tip.append('text')
            .attr('y', 120)
            .attr('class', 'fig1_tool_tip_state')
            .attr("fill", "#FFFFFF")
            .style('font-family', 'Roboto Slab')
        ...
        vis.tool_tip.append('text')
            .attr('y', 140)
            .attr('class', 'fig1_tool_tip_date')
            .attr("fill", "#FFFFFF")
            .style('font-family', 'Roboto Slab')

        vis.tool_tip.append('text')
            .attr('y', 160)
            .attr('class', 'fig1_tool_tip_metric')
            .attr("fill", "#FFFFFF")
            .style('font-family', 'Roboto Slab')
    ...
    wrangleData() {
        ...
        // Create dictionary object of state colors, using colorArray found in fig1_utils.js
        ...
        autocomplete("fig1_automenu", vis.displayStates);
    ...
    updateVis() {
        ...
        vis.lines = vis.svg.selectAll(".line")
            ...
            .join("path")
                ...
                .attr('class', function(d) {return `fig1_line_${d[0]}`})
        ...
        vis.field
            ...
            .on('mouseover', function () {
                if (fig1_selection != 'None') {
                    d3.select('.fig1_tool_tip').attr('display', null)
                    vis.tool_tip_line.attr('stroke', vis.colors[fig1_selection])
                    vis.tool_tip_dot.attr('stroke', vis.colors[fig1_selection])
                }})
            // On mouseout of chart, hide tooltip
            .on('mouseout', function () {d3.select('.fig1_tool_tip').attr('display', 'none')})
            // On moving mouse within chart, if line selected, make tooltip follow mouse and show information for selected state
            .on('mousemove', function(event, d) {
                if (fig1_selection != 'None') {
                    d3.select('.fig1_tool_tip')
                    ...
                    vis.data_index = vis.bisectDate(vis.displayData.get(fig1_selection), vis.tooltip_date)
                    d3.select('.fig1_tool_tip_state').text(stateAbbrev[fig1_selection]).attr('text-anchor', 'start').attr('x', 10)
                    d3.select('.fig1_tool_tip_date').text(`${weekDays[vis.tooltip_date.getDay()]}, ${d3.timeFormat('%e %B %Y')(vis.tooltip_date)}`).attr('text-anchor', 'start').attr('x', 10)
                    d3.select('.fig1_tool_tip_metric').text(`${'DEX (7-Day Moving Average)'}: ${Math.round(d3.format(',')(vis.displayData.get(fig1_selection)[vis.data_index][vis.metric]))}`).attr('text-anchor', 'start').attr('x', 10)
                    if (vis.data_index > 270) {
                        d3.select('.fig1_tool_tip_state').attr('text-anchor', 'end').attr('x', -10)
                        d3.select('.fig1_tool_tip_date').attr('text-anchor', 'end').attr('x', -10)
                        d3.select('.fig1_tool_tip_metric').attr('text-anchor', 'end').attr('x', -10)
                    }
                    vis.tool_tip_dot.attr('cy', vis.y(vis.displayData.get(fig1_selection)[vis.data_index][vis.metric]))
                }
            })
